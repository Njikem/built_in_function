...
insert into order_item(oreder_id, item_id) values(4, 8);
...
select name from customer where order=5;

select username, count(id) from customer where customer.customer_id;

select item_id, AVG(price) from item where item group by item_id;

select username, sum(price) from item 
